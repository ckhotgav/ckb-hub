...
如果你想要修改這 18 個開關的指令描述，或是新增自己的超強提示詞，請修改以下三個構成工具箱靈魂的地方：

1. **`main.py`**：控制後端的預設狀態，尋找 `default_status` 字典，修改裡面的 ID 名稱。
2. **`static/index.html`**：控制網頁前端。尋找 `<div class="skill-card">` 區塊，修改標題、敘述文字。
3. **`skills/` 資料夾**：這是 AI 的「規則儲藏室」！只要你在這個資料夾裡，建立一個與開關 ID 同名的 Markdown 檔案（例如 `gas_deploy.md`），當開關打開時，AI 就會自動把你寫在裡面的長篇大論讀進腦袋裡！
