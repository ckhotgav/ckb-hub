...
## 🔧 進階：我要怎麼加掛新技能？ (全自動外掛尋找模式)

CKB-Hub 具備極致的「熱插拔」擴充能力！你不必修改任何 Python 或 HTML 程式碼，系統會全自動幫你搞定。

如果你想要新增一個「AWS 部署」或是「小紅書文案產生器」的技能，請遵循以下超簡單步驟：

1. **複製樣板**：進入 `skills/` 資料夾，複製 `SKILL_TEMPLATE.md`。
2. **填寫身分證**：將複製出來的檔案改名（例如 `aws_deploy.md`），打開它，並在最上方的 YAML 區塊填寫你要顯示在網頁上的名稱與分類：
   ```yaml
   ---
   id: aws_deploy
   title: AWS 自動部署
   description: 幫我將程式碼上傳到 AWS 雲端。
   category: 靜態與主機部署
   ---
   ```
3. **撰寫神級指令**：在 YAML 區塊下方，用 Markdown 寫下你希望 AI 遵守的規則與對話劇本。
4. **大功告成**：存檔後，重新整理你的 CKB-Hub 網頁。你會發現網頁上已經**自動長出了新按鈕**，而且 AI 也已經**自動學會了這個新 API**！
...
graph TD
    User[使用者] -->|想要加掛新功能| A
    A[複製 SKILL_TEMPLATE.md] --> B[建立新技能 Markdown 檔]
    B --> C[填寫 YAML 身分證與 AI 提示詞]
    C -->|存檔並放入| D[skills/ 目錄]
    D -->|重新整理網頁| E{CKB-Hub 動態掃描}
    
    E -->|讀取 YAML| F[網頁全自動長出新開關]
    E -->|讀取 YAML| G[MCP 伺服器全自動註冊新 API]
